...
export default function BuyerRequestsPage() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/marketplace/farmer/requests`, {
  ...
  const handleAction = async (requestId: string, action: 'accept' | 'reject') => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/marketplace/request/${requestId}/${action}`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleCounterOffer = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/marketplace/request/${selectedRequest._id}/counter`, {
        method: 'PUT',
        headers: {
