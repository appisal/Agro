...
export default function CropPlanningPage() {
  ...
  const fetchCrops = async () => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/lifecycle/active`, {
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          const token = getToken();
          const res = await fetch(`${import.meta.env.VITE_API_URL}/crop/weather?lat=${lat}&lng=${lng}`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
          if (res.ok) {
            const data = await res.json();
  ...
  const handlePredict = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL}/crop/predict`, {
        method: 'POST',
  ...
  const handleDeleteCrop = async (id: string) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL}/lifecycle/active/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL}/lifecycle/active/${selectedCrop._id}/expense`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleTransferSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL}/lifecycle/active/${transferCrop._id}/transfer`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({
