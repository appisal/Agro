...
export default function DashboardPage() {
  ...
  const fetchCrops = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/lifecycle/active`, {
  ...
  const fetchStorage = async () => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/storage/${user._id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchOrders = async () => {
    try {
      const token = getToken();
      const endpoint = user?.role === 'buyer' ? `${API_URL}/marketplace/my-requests` : null;
      if (!endpoint) return;
  ...
  const handleDeleteCrop = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/lifecycle/active/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/lifecycle/active/${selectedCrop._id}/expense`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleTransferToShelf = async (cropId: string, e: React.MouseEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/lifecycle/active/${cropId}/transfer`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleViewHistory = async (crop: any, e: React.MouseEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/lifecycle/active/${crop._id}/expenses`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
