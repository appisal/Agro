...
export default function DataAnalystPage() {
  ...
  const fetchAllData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const activeRes = await fetch(`${API_URL}/lifecycle/active`, { headers });
      const activeData = activeRes.ok ? await activeRes.json() : [];

      // 2. Shelf monitoring crops (selling)
      const shelfRes = await fetch(`${API_URL}/lifecycle/shelf`, { headers });
      const shelfData = shelfRes.ok ? await shelfRes.json() : [];

      // 3. Archived crops (completed)
      const archivedRes = await fetch(`${API_URL}/lifecycle/archived`, { headers });
  ...
  const syncDataToSheets = async () => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/analytics/sync-sheets`, {
        method: 'POST',
