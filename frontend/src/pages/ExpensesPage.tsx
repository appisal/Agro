...
export default function ExpensesPage() {
    ...
    const fetchLogs = async () => {
        ...
        try {
            ...
            // Fetch Expenses
            const expRes = await fetch(`${import.meta.env.VITE_API_URL}/crop/${selectedCropId}/expense`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const expenses = expRes.ok ? await expRes.json() : [];

            // Fetch Income
            const incRes = await fetch(`${import.meta.env.VITE_API_URL}/crop/${selectedCropId}/income`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    const handleAddExpense = async (e: React.FormEvent) => {
        ...
        try {
            const token = getToken();
            const res = await fetch(`${import.meta.env.VITE_API_URL}/crop/${selectedCropId}/expense`, {
                method: 'POST',
    ...
    const handleAddIncome = async (e: React.FormEvent) => {
        ...
        try {
            const token = getToken();
            const res = await fetch(`${import.meta.env.VITE_API_URL}/crop/${selectedCropId}/income`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
