...
export default function FarmerTalkPage() {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/farmer-talk/questions?crop=${filterCrop === 'All' ? 'All' : filterCrop}&sort=${sortOrder}`, {
  ...
  const loadQuestionDetail = async (questionId: string) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/farmer-talk/question/${questionId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleAskSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/farmer-talk/question`, {
        method: 'POST',
        headers: {
  ...
  const handleReplySubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/farmer-talk/reply`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
