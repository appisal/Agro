...
export default function MarketplacePage() {
  ...
  const handleAcceptRequest = async (requestId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/marketplace/request/${requestId}/accept`, {
        method: 'PUT',
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/marketplace/request/${selectedRequest._id}/message`, {
        method: 'PUT',
        headers: { 
  ...
  const fetchCrops = async () => {
    try {
      setLoading(true);
      const token = getToken();
      const res = await fetch(`${API_URL}/marketplace/crops`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchMyRequests = async () => {
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/marketplace/my-requests`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/marketplace/request`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
