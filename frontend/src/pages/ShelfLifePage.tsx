...
export default function ShelfLifePage() {
  ...
  const fetchStorage = async () => {
    ...
    try {
      ...
      const res1 = await fetch(`${API_URL}/storage/${user._id}`, {
      ...
      const res2 = await fetch(`${API_URL}/lifecycle/shelf`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      let shelfData = [];
      if (res2.ok) {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/storage/add`, {
        method: 'POST',
        headers: {
  ...
  const handleSellSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = selectedItemForSale.isLifecycle 
        ? `${API_URL}/lifecycle/shelf/${selectedItemForSale._id}/complete`
        : `${API_URL}/storage/${selectedItemForSale._id}/sell`;
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/storage/${selectedItemForEdit._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/storage/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
